...
public class Puzzle2 {
    ...
    private static void addAntinodes(Pos antennaPos, Pos nextMatchingAntenna, char[][] grid, Set<Pos> antinodes) {
        ...
        Pos antinodeNear = new Pos(antennaPos.x , antennaPos.y);
        while (antinodeExists(antinodeNear, grid)) {
            antinodes.add(antinodeNear);
            antinodeNear = new Pos(antinodeNear.x - deltaX, antinodeNear.y - deltaY);
        }


        Pos antinodeFar = new Pos(nextMatchingAntenna.x, nextMatchingAntenna.y);
        while (antinodeExists(antinodeFar, grid)) {
            antinodes.add(antinodeFar);
            antinodeFar = new Pos(antinodeFar.x + deltaX, antinodeFar.y + deltaY);
        }
    }
